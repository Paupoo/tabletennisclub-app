...
<div class="flex flew-row flex-wrap gap-2">
    <div>
        <x-form.field name="season_id" :label="__('Select a season')">
            <x-select-input class="block w-fit mt-1" id="season" name="season_id" required autofocus>
                @foreach ($seasons as $season)
                    <option
                        value="{{ $season->id }}" @selected($season->id === $team->season?->id)>{{ $season->name }}</option>
                @endforeach
            </x-select-input>
        </x-form.field>
    </div>
    ...
    <div>
        <x-form.field name="category" :label="__('Select a category')">
            <x-select-input class="block w-fit mt-1" id="category" name="category" required autofocus>
                @foreach ($league_categories as $category)
                    <option
                        value="{{ $category->name }}" @selected($category->name === $team->league?->category)>{{ $category->value }}</option>
                @endforeach
            </x-select-input>
        </x-form.field>
    </div>
    ...
    <div>
        <x-form.field name="level" :label="__('Select a level')">
    ...
    <div>
        <x-form.field name="division" :label="__('Division')">
...
<div>
    <x-form.field name="name" :label="__('Select a letter')">
        <x-select-input id="name" name="name" type="text" class="block w-full mt-1" required autofocus
            ...
            @foreach ($team_names as $team_name)
                <option
                    value="{{ $team_name->name }}" @selected(old('name', $team?->name) === $team_name->name)>{{ $team_name->name }}</option>
            @endforeach
        </x-select-input>
    </x-form.field>
</div>
                ...
                <tr class="table-row hover:bg-gray-100">
                    ...
                    <td class="table-cell text-right w-fit">
                        <x-checkbox-input id="player{{ $loop->iteration }}" name="players[]" :value="$user->id"
                            :checked="in_array($user->id, $attachedUsers)" />
                        <x-input-error class="mt-2" :messages="$errors->get('players.' . $loop->index)"/>
                    </td>
                    <td class="table-cell text-right w-fit">
                        <x-radio-input name="captain_id" :value="$user->id"
                            :checked="$user->id === $team->captain_id" />
                    </td>
                </tr>
            @endforeach
        @endif
