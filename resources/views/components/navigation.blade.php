<nav class="{{ $fixed ?? true ? 'fixed' : '' }} w-full bg-white/95 backdrop-blur-xs z-50 shadow-xs" x-data="navigation">
        ...
        <div class="flex justify-between items-center h-16">
            ...
            <div class="hidden md:block">
                <div class="ml-10 flex items-baseline space-x-4">
                    ...
                    <a href="{{ route('news.index') }}" class="text-gray-900 hover:text-club-blue px-3 py-2 rounded-md text-sm font-medium transition-colors {{ request()->routeIs('events') ? 'text-club-blue' : '' }}">
                        Nouvelles
                    </a>
    ...
    <div x-show="mobileMenuOpen" x-transition @click.away="closeMobileMenu()" class="md:hidden bg-white border-t">
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            <a href="{{ route('news.index') }}" @click="closeMobileMenu()" class="block text-gray-900 hover:text-club-blue px-3 py-2 rounded-md text-base font-medium">Nouvelles</a>
