...
| # | Constat | Enjeu |
|---|---|---|
| I1 | **La page Paiements héberge une copie dégradée de l'import bancaire.** `⚡payments/processImport()` : `catch (Exception) { continue; }` (lignes perdues en silence, seul `$importedCount` est annoncé) **et aucune déduplication** — réimporter le même extrait recrée tout en double. **Or `⚡transactions/processImport()` fait tout correctement** : `import_fingerprint` SHA-256, comptage new/duplicate/error, `failed_rows` (ligne + données + motif), journal `BankImport`, toast en `warning` dès qu'il y a une erreur. Deux imports vers la même table `transactions`, un bon, un piège. | **Intégrité comptable.** Ce n'est pas « ajouter un rapport d'erreurs » : le bon code existe déjà une page plus loin. **Correctif : supprimer l'import de la page Paiements** (ou le rediriger vers Transactions). `rapprocher-les-paiements.md` porte l'avertissement en attendant — **à retirer une fois fait**. |
| I2 | **Recherche de remplaçant : filtrage muet.** `⚡captain-selection` filtre par catégorie d'équipe (genre, 40+ pour vétérans) et retire les joueurs déjà alignés — sans un mot. Un sélectionneur cherche un joueur dont il est sûr, n'obtient rien, et n'a aucune explication. | Confusion garantie. Correctif : dire pourquoi la liste est vide. |
| I3 | **Une amende est irréversible.** `IssueFine` est la **seule** action du domaine : pas d'annulation, pas de suppression, pas de `SoftDeletes` sur `Fine`, pas de brouillon. Valider crée le paiement **et** notifie le membre (+ ses tuteurs s'il est mineur) d'un bloc. | **Le trésorier se trompera de membre un jour.** Il n'aura aucun recours dans l'app. Correctif minimal : annulation d'une amende (avec annulation du paiement), ou au moins une confirmation récapitulative avant envoi. |
| I4 | **Ouvrir le PV vole le stylo.** `⚡minutes/minutes.php:mount()` appelle `acquireMinutesLock()` sans rien demander : venir lire dépossède celui qui écrivait. | Mineur (se répare seul en 10 min, reprise possible), mais surprenant. Correctif : ne prendre le verrou qu'à la première frappe. |
| I5 | **Une inscription tournoi non payée est annulée sans prévenir le membre.** `TournamentService::expirePaymentDeadlines()` fait un `update(['registration_status' => 'cancelled'])` brut puis `openSpot()` — **aucune notification**. Asymétrie flagrante avec `expireConfirmationDeadlines()`, juste au-dessus, qui notifie (`TournamentConfirmationExpiredNotification`). La `TournamentRegistrationCancelledNotification` existe mais ne sert que dans le chemin d'annulation volontaire. | Le membre reçoit des rappels **avant**, puis perd sa place en silence et le découvre le jour du tournoi. Ressemble à un oubli, pas à un choix. Correctif : notifier à l'expiration, comme le fait déjà la liste d'attente. |
