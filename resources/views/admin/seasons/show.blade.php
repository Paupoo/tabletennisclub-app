    ...
    <div x-data="{ showSubscribeModal: false }">
        <x-admin-block>
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                ...
                <!-- Wrapper avec overflow-visible pour permettre au dropdown de dépasser -->
                                ...
                                <tr class="hover:bg-gray-50" x-data="{ showActions: false }">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">
                                            {{ $subscription->user->fullName }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                        <div class="flex flex-col gap-2">
                                            ...
                                            <div class="flex flex-wrap justify-end gap-x-2 gap-y-2">
                                                @foreach ($subscription->availableTransitions() as $action => $label)
                                                    <form method="POST" action="{{ route('admin.subscriptions.' . $action, $subscription) }}">
                                                        @csrf
                                                        <button type="submit"
                                                            class="px-2 py-1 border border-gray-300 rounded text-gray-700 text-xs hover:bg-gray-100">
                                                            {{ __($label) }}
                                                        </button>
                                                    </form>
                                                @endforeach
                                            ...
                                            <div class="flex flex-wrap justify-end gap-x-2 gap-y-2">
                                                ...
                                                <form method="POST" action="{{ route('admin.subscriptions.delete', $subscription) }}">
                                                    @csrf
                                                    <button type="submit"
                                                        class="px-2 py-1 border border-gray-300 rounded text-gray-700 text-xs hover:bg-gray-100">
                                                        {{ __('Soft delete') }}
                                                ...
                                                <form method="POST" action="{{ route('admin.subscriptions.unsubscribe', [$season, $subscription->user]) }}">
                                                    @csrf
                                                    <button type="submit"
                                                        class="px-2 py-1 border border-gray-300 rounded text-red-700 text-xs hover:bg-red-50 font-medium">
