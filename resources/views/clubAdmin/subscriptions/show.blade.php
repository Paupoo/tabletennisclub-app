        ...
        <x-admin-block>
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    @endphp
                    <span
                        class="px-3 py-1 inline-flex text-sm font-semibold rounded-full {{ $statusColors[$status] ?? 'bg-gray-100 text-gray-800' }}">
                        {{ __(ucfirst($status)) }}
                    </span>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div class="bg-gray-50 rounded-lg p-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">
                            {{ number_format($subscription->amount_due, 2) }}&nbsp;€
                        </p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
                ...
                <form method="POST" action="{{ route('clubAdmin.subscriptions.addTrainingPack', $subscription) }}">
                    @csrf
                    <div class="space-y-3">
                        ...
                        <div
                            ...
                            <div class="flex items-center">
                                <x-checkbox-input name="training_packs[]" value="{{ $trainingPack->id }}"
                                    id="training-pack-{{ $trainingPack->id }}" :disabled="$subscription->status !== 'pending'"
                                    :checked="$subscription->trainingPacks->contains($trainingPack->id)" class="mr-3" />
                            ...
                            <span
                                class="text-sm text-gray-600">{{ number_format($trainingPack->price, 2) }}&nbsp;€</span>
                            @endif
                        </div>
                        @empty
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <form method="POST" action="{{ route('clubAdmin.subscriptions.' . $action, $subscription) }}"
                        class="inline">
                        @csrf
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
                ...
                <div class="space-y-3 mb-4">
                    ...
                    <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
                        <div>
                            ...
                            <p class="text-xs text-gray-500">{{ $payment->created_at->format('d/m/Y H:i') }}
                            </p>
                        </div>
                        @if ($payment->paid_at)
                ...
                <div class="flex flex-wrap gap-3 justify-left items-center">
                    @if ($subscription->payments->count() <= 0 && $subscription->status == 'confirmed')
                        ...
                        @if ($subscription->payments->count() > 0 && $subscription->status == 'confirmed')
                        <form method="POST" action="{{ route('clubAdmin.subscriptions.sendPaymentInvite') }}"
                            class="inline">
                            @csrf
                    ...
                    <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                        <div>
                            ...
                            <p class="text-xs text-gray-500">
                                {{ __('Mark this subscription as deleted (can be restored)') }}
                            </p>
                        </div>
                        <form method="POST" action="{{ route('clubAdmin.subscriptions.unsubscribe', $subscription) }}"
                            class="inline">
                            @csrf
                    ...
                    <div class="flex items-center justify-between p-4 border border-red-200 rounded-lg bg-red-50">
                        <div>
                            ...
                            <p class="text-xs text-red-600">
                                {{ __('Permanently delete this subscription (cannot be undone)') }}
                            </p>
                        </div>
                        <form method="POST" action="{{ route('clubAdmin.subscriptions.destroy', $subscription) }}"
                            class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
