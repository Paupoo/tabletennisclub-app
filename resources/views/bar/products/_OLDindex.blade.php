              ...
              <form action="{{ route('bar.products.store') }}" method="POST" class="product-create">
                ...
                <div class="product-create-grid">
                    <div>
                        ...
                        <input class="product-input" name="product_name" value="{{ old('product_name') }}" placeholder="ex: Eau plate" required>
                    </div>
                    <div>
                        ...
                        <div class="select-with-plus">
                            <select class="product-input" name="category_id" id="category_id_create" required>
                                ...
                                {{ session('selected_category_id') == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                              </option>
                              @endforeach
                            </select>
                        <a href="{{ route('bar.categories.index') }}" class="btn-plus" id="add-cat-link" aria-label="{{ __('Add a category') }}">+</a>
                        </div>
                    </div>
            ...
            <div class="product-compact-row">
              <div class="product-field product-field--price">
                ...
                <input class="product-input"
                       name="sale_price"
                       inputmode="decimal"
                       value="{{ number_format($p->sale_price / 100, 2, ',', '') }}">
              </div>
...
<form method="POST" action="{{ route('bar.products.destroy', $p) }}" style="flex:1; margin:0;">
    ...
    <button 
        ...
        onclick="return {{ $stock === 0 ? 'confirm(\'Supprimer ce produit : ' . $p->name . ' ?\')' : 'false' }}"
    >
        🗑 Supprimer
    </button>
          ...
          {{-- DELETE BUTTON (always visible, disabled if stock > 0) --}}
          <form method="POST" action="{{ route('bar.products.destroy', $p) }}" style="margin-top:6px;">
            @csrf
            @method('DELETE')
            <button class="btn btn-clear btn-block" type="submit" {{ $stock > 0 ? 'disabled' : '' }}
              title="{{ $stock > 0 ? 'Stock non nul : suppression impossible' : '' }}"
              onclick="return {{ $stock === 0 ? 'confirm(\'Supprimer ce produit ?\')' : 'false' }}">
              🗑 Supprimer
            </button>
          </form> -->
          </form>
