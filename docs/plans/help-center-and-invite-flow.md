...
- [x] **Matériel / clés** : `suivre-le-materiel-confie.md` (2026-07-17). Angle « qui détient quoi » plutôt que champ-par-champ. Règle que l'ancien manuel n'explicitait pas : trousseau modifiable par **tout le comité** (fiche membre), mais détenteur de caisse réservé à **trésorier + admin** (pas le président). ⚠️ Caduc depuis l'inventaire des trousseaux : la case de la fiche membre a disparu, et modifier un trousseau demande désormais la délégation **Installations**. L'ancien `manual-equipment-fr.md` était exact sur les filtres — vérifié.
...
> ### ✅ I6 — Le mythe « 72h » — **CORRIGÉ le 2026-07-17**
>
> **Fait.** Le « 72h » traînait à **4 endroits** ; le plus grave n'était pas un commentaire mais **l'e-mail de demande de paiement** (`tournament-payment-request.blade.php`), qui mentait au membre (« Sans paiement dans les 72h ») alors que la date réelle est affichée juste au-dessus. Corrigé en « au terme de ce délai » (testable → 1 test rouge/vert : plus de « 72h », la vraie date reste). Les 2 commentaires trompeurs (`TournamentService::expirePaymentDeadlines` docblock, `Kernel.php`) réécrits pour dire la vraie règle. Le 4ᵉ (article d'aide) **débunkait déjà** le mythe — laissé tel quel. Bonus : **3 tests de caractérisation** verrouillent la vraie règle du délai (date de clôture / +3 j si tardif / aucun le jour même) — elle n'était couverte par aucun test.
>
> <details><summary>Constat d'origine (conservé)</summary>
>
