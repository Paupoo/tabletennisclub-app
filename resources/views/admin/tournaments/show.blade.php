        ...
        <x-admin-block2>
            @if(count($matches) > 0)
                @foreach($matches as $match)
                    <p>{{ $match->pool->name }}</p>
                    <p>{{ $match->player1->first_name }} {{ $match->player1->last_name }} vs {{ $match->player2->first_name }} {{ $match->player2->last_name }} </p>
                @endforeach
            @else
            <p>{{ __('No match generated yet.') }}
            @endif
        </x-admin-block2>
    </div>
</x-app-layout>
