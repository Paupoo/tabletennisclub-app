                    ...
                    <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                        ...
                        <button @click="showStats = !showStats"
                            ...
                            <svg x-bind:class="showStats ? 'rotate-180' : 'rotate-0'"
                                class="w-4 h-4 mr-2 transition-transform duration-200" fill="none"
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 align-top">
                                    ...
                                    <td class="px-4 py-2 whitespace-nowrap">
                                        ...
                                        <span
                                            class="px-2 py-0.5 inline-flex text-xs font-semibold rounded-full bg-club-yellow text-club-blue">
                                            {{ __('Competitive') }}
                                        </span>
                                        @else
                                        <span
                                            class="px-2 py-0.5 inline-flex text-xs font-semibold rounded-full bg-gray-100 text-gray-800">
                                            {{ __('Casual') }}
                                        </span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-4 py-2 whitespace-nowrap">
                                        ...
                                        @endphp
                                        <span
                                            class="px-2 py-0.5 inline-flex text-xs font-semibold rounded-full {{ $statusColors[$status] ?? 'bg-gray-100 text-gray-800' }}">
                                            {{ __(ucfirst($status)) }}
                                        </span>
                                    </td>
                            ...
                            <div class="mt-3 pt-3 border-t border-gray-200">
                                ...
                                <button @click="showActions = !showActions"
                                    ...
                                    <svg class="w-5 h-5 transition-transform"
                                        :class="{ 'rotate-180': showActions }" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div>
                                        ...
                                        <form method="POST"
                                            ...
                                            <div class="flex flex-row gap-4 justify-start">
                                                ...
                                                <x-input-label
                                                    for="{{ $trainingPack->id }}">{{ $trainingPack->name }}</x-input-label>
                                            </div>
                                            @endforeach
                                            @if (collect($subscription->availableTransitions())->has('confirm'))
                                            <x-primary-button
                                                class="w-fit">{{ __('Save') }}</x-primary-button>
                                            @endif
                                        </form>
                                    </div>
                                    @endif
        ...
        <div x-show="showSubscribeModal" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                <div x-show="showSubscribeModal" @click.outside="showSubscribeModal = false"
                    ...
                    <form action="{{ route('clubEvents.interclubs.seasons.subscribe', $season) }}" method="POST" class="space-y-4">
                        @csrf
