...
function expenseHelpSlugsFor(User $user): array
{
...
it('shows adults how to declare, and never a minor', function (): void {
    expect(expenseHelpSlugsFor(User::factory()->create()))->toContain('declarer-une-note-de-frais')
        ->and(expenseHelpSlugsFor(User::factory()->minor()->create()))->not->toContain('declarer-une-note-de-frais');
});

it('shows every decider how to decide, the backup délégation included', function (Role $role): void {
    expect(expenseHelpSlugsFor(User::factory()->withRole($role)->create()))->toContain('traiter-les-notes-de-frais');
...
it('keeps the deciding task from a committee member who only reads', function (): void {
    expect(expenseHelpSlugsFor(User::factory()->isCommitteeMember()->create()))->not->toContain('traiter-les-notes-de-frais');
});

it('shows the auditors, and the committee, how to read the reports', function (Role $role): void {
    expect(expenseHelpSlugsFor(User::factory()->withRole($role)->create()))->toContain('verifier-les-notes-de-frais');
})->with([Role::ACCOUNTS_AUDIT, Role::COMMITTEE]);

it('keeps the auditing task from a plain member', function (): void {
    expect(expenseHelpSlugsFor(User::factory()->create()))->not->toContain('verifier-les-notes-de-frais');
});

it('hides the three articles when the feature is off', function (): void {
    config(['features.expense_reports' => false]);

    expect(expenseHelpSlugsFor(User::factory()->isAdmin()->create()))
