        ...
        <x-admin-block>
            ...
            <div class="bg-white rounded-lg shadow-lg p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6">
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start space-y-3 sm:space-y-0">
                    <div>
                        <h2 class="text-lg sm:text-xl lg:text-2xl font-bold text-club-blue mb-1 sm:mb-2">
                            {{ __('Trainings') }}</h2>
                        <p class="text-gray-600 text-xs sm:text-sm lg:text-base">
                            {{ __('Manage training sessions and schedules for your club members.') }}</p>
                    </div>
                    ...
                    <div class="flex flex-col xs:flex-row space-y-2 xs:space-y-0 xs:space-x-2 lg:space-x-3">
                        ...
                        <button @click="showStats = !showStats"
                            ...
                            <svg x-bind:class="showStats ? 'rotate-180' : 'rotate-0'"
                            ...
                            <span class="hidden sm:inline"
                                x-text="showStats ? '{{ __('Hide Statistics') }}' : '{{ __('Show Statistics') }}'"></span>
                            <span class="sm:hidden"
                                x-text="showStats ? '{{ __('Stats') }}' : '{{ __('Stats') }}'"></span>
                        </button>
                        ...
                        <button @click="showFilters = !showFilters"
                            ...
                            <span class="hidden sm:inline"
                                x-text="showFilters ? '{{ __('Hide Filters') }}' : '{{ __('Show Filters') }}'"></span>
                            <span class="sm:hidden"
                                x-text="showFilters ? '{{ __('Filters') }}' : '{{ __('Filters') }}'"></span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div x-show="showStats" x-transition:enter="transition ease-out duration-300"
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
                    <div
                        class="bg-club-blue bg-opacity-10 rounded-lg p-2 sm:p-4 text-center border border-club-blue border-opacity-20">
                        <div class="text-base sm:text-xl lg:text-2xl font-bold text-club-yellow">
                            {{ $stats?->get('totalActiveUsers') ?? 0 }}</div>
                    ...
                    <div class="bg-club-yellow rounded-lg p-2 sm:p-4 text-center border border-club-yellow">
                        <div class="text-base sm:text-xl lg:text-2xl font-bold text-club-blue">
                            {{ $stats?->get('totalCompetitors') ?? 0 }}</div>
                    ...
                    <div class="bg-blue-50 rounded-lg p-2 sm:p-4 text-center border border-blue-200">
                        <div class="text-base sm:text-xl lg:text-2xl font-bold text-blue-600">
                            {{ $stats?->get('totalUsersCreatedLastYear') ?? 0 }}</div>
                        <div class="text-xs sm:text-sm text-blue-700 font-medium">{{ __('Total Directed Trainings') }}
                        </div>
                    </div>
                    <div class="bg-yellow-50 rounded-lg p-2 sm:p-4 text-center border border-yellow-200">
                        <div class="text-base sm:text-xl lg:text-2xl font-bold text-yellow-600">
                            {{ $stats?->get('totalUnpaidUsers') ?? 0 }}</div>
                        <div class="text-xs sm:text-sm text-yellow-700 font-medium">{{ __('Average occupation') }}
                        </div>
                    </div>
            ...
            <div x-show="showFilters" x-transition:enter="transition ease-out duration-300"
                ...
                x-transition:leave-end="opacity-0 transform -translate-y-2"
                class="bg-white rounded-lg shadow-lg p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6">
                <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4">{{ __('Filter Trainings') }}
                </h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    <div>
                        <label
                            class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">{{ __('Training Type') }}</label>
                        <select
                            class="w-full rounded-lg border-gray-300 focus:border-club-blue focus:ring-club-blue text-sm">
                    ...
                    <div>
                        <label
                            class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">{{ __('Level') }}</label>
                        <select
                            class="w-full rounded-lg border-gray-300 focus:border-club-blue focus:ring-club-blue text-sm">
                    ...
                    <div>
                        <label
                            class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">{{ __('Date Range') }}</label>
                        <input type="date"
                            class="w-full rounded-lg border-gray-300 focus:border-club-blue focus:ring-club-blue text-sm">
                    </div>
                    <div class="flex items-end">
                        <button
                            class="w-full bg-club-blue hover:bg-club-blue-light text-white px-3 py-2 rounded-lg font-medium transition-colors text-sm">
                            {{ __('Apply Filters') }}
                        </button>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                @if ($trainingPacks->count() == 0)
                    ...
                    <div class="text-center py-8 sm:py-12 px-4">
                        <div
                            class="mx-auto w-16 h-16 sm:w-24 sm:h-24 bg-gray-100 rounded-full flex items-center justify-center mb-3 sm:mb-4">
                            <svg class="w-8 h-8 sm:w-12 sm:h-12 text-gray-400" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 9a2 2 0 00-2 2v2a2 2 0 002 2m0 0h14m-14 0a2 2 0 002 2v2a2 2 0 01-2 2M7 7V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10">
                                </path>
                            </svg>
                        </div>
                        <h3 class="text-base sm:text-lg font-medium text-gray-900 mb-2">
                            {{ __('No trainings scheduled') }}</h3>
                        <p class="text-gray-600 mb-4 sm:mb-6 text-sm sm:text-base">
                            {{ __('It seems that no trainings have been defined. Start by creating a new training session.') }}
                        </p>
                        <a href="{{ route('trainings.create') }}"
                    ...
                    <div class="hidden lg:block overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($trainingPacks as $training)
                                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">
                                                ...
                                                @endif
                                                <span
                                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-club-yellow bg-opacity-20 text-club-blue">
                                                    {{ $training->level }}
                                                </span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div
                                                    class="flex-shrink-0 w-8 h-8 bg-green-100 rounded-full flex items-center justify-center mr-2">
                                ...
                                <div class="flex items-center justify-between text-sm">
                                    <div class="flex items-center space-x-4 text-gray-600">
                                        ...
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-club-yellow bg-opacity-20 text-club-blue">
                                                {{ $training->level }}
                                            </span>
                                        </div>
                                        ...
                                        @if ($training->trainer)
                                            <div class="hidden sm:flex items-center">
                                                ...
                                                <span
                                                    class="text-xs text-gray-500">{{ $training->trainer->last_name }}</span>
                                            </div>
                                        @endif
                                    </div>
