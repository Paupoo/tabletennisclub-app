            ...
            <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <select wire:model.live="status"
                            class="border border-gray-300 pr-8 rounded-lg px-3 py-2 text-sm focus:ring-club-blue focus:border-club-blue">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <select wire:model.live="interest"
                            class="border border-gray-300 rounded-lg px-3 pr-8 py-2 text-sm focus:ring-club-blue focus:border-club-blue">
                        <option value="">{{ __('All') }}</option>
                        @foreach (ContactReasonEnum::cases() as $contactReason)
                            <option value="{{ $contactReason->name }}">{{ $contactReason->getLabel() }}</option>
                        @endforeach
                    </select>
                </div>
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="px-4 sm:px-6 py-4 border-b border-gray-200 bg-gray-50">
            <h3 class="text-lg font-medium text-gray-900">
                {{ __('Contacts') }}
                <span class="text-sm font-normal text-gray-500">({{ $contacts->total() }} {{ __('result(s)') }})</span>
            </h3>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                @foreach ($contacts as $contact)
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div
                                class="text-sm text-gray-900">{{ $contact->interest?->getLabel() ?? 'Non spécifié' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $contact->created_at->format('d/m/Y') }}
                            <div
                                class="text-xs text-gray-400">{{ $contact->created_at->timezone('Europe/Brussels')->format('H:i') }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php
                                $statusConfig = [
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="{{ route('clubAdmin.contacts.show', $contact) }}"
        ...
        <!-- Display this message if no results -->
        @if($contacts->isEmpty())
